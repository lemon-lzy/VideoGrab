    ...
    try:
        ...
        if ffmpeg_exe and os.path.exists(ffmpeg_exe):
            ...
            if not os.path.exists(target_exe):
                import shutil as sh
                sh.copy2(ffmpeg_exe, target_exe)
            return ffmpeg_dir
    except Exception:
        pass
    return None
...
class VideoDownloader:
    ...
    def download_video(self, url: str, format_id: str) -> dict:
        ...
        if has_ffmpeg:
            filepath, filename = self._convert_to_h264(filepath, filename, ffmpeg_path)
    ...
    def _convert_to_h264(self, filepath: str, filename: str, ffmpeg_path: str) -> tuple:
        """将视频转换为 H.264 编码以提高兼容性"""
        import subprocess
        ...
        try:
            ...
            if 'hevc' not in output.lower() and 'h265' not in output.lower():
                return filepath, filename
        except Exception:
            return filepath, filename

        converted_path = filepath.replace(".mp4", "_h264.mp4")
        converted_filename = filename.replace(".mp4", "_h264.mp4")
        
        try:
            subprocess.run(
                [ffmpeg_exe, "-i", filepath, "-c:v", "libx264", "-preset", "fast",
                 "-c:a", "copy", "-movflags", "+faststart", "-y", converted_path],
                capture_output=True, timeout=300
            )
            if os.path.exists(converted_path) and os.path.getsize(converted_path) > 0:
                os.remove(filepath)
                return converted_path, converted_filename
        except Exception:
            pass
